...
description: Complete guide to deploying Redis.do on Cloudflare Workers with wrangler.jsonc configuration, environments, secrets, and production best practices.
---

# Cloudflare Deployment Guide

This guide covers everything you need to deploy Redis.do to Cloudflare Workers, from initial setup to production configuration.

## Prerequisites

Before deploying Redis.do, ensure you have:
...
The fastest way to deploy Redis.do:

```bash
# Clone the repository
git clone https://github.com/nathanclevenger/redis.do.git
cd redis.do
...
Install Redis.do as a dependency:
...
The `wrangler.jsonc` file is the core configuration for your Redis.do deployment. Here is a complete, annotated configuration:
...
```jsonc
{
  ...
  // Durable Objects - the core of Redis.do
  "durable_objects": {
    "bindings": [
      {
        "name": "REDIS_SHARDS",
...
Protect your Redis.do instance with bearer token authentication:
...
```jsonc
{
  ...
  "env": {
    ...
    "staging": {
      "name": "redis.do-staging",
      "vars": {
        "ENVIRONMENT": "staging"
      }
    },
    "production": {
      "name": "redis.do-production",
      "vars": {
        "ENVIRONMENT": "production"
      }
    }
...
<Callout type="info">
Redis.do distributes data across 256 shards, giving you an effective storage limit of 2.56TB (256 x 10GB). In practice, you will hit request limits before storage limits.
</Callout>
