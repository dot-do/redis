...
description: JavaScript/TypeScript client library for Redis.do with ioredis-compatible API and pipeline support.
---

# Client Library Documentation

Redis.do provides a JavaScript/TypeScript client library with an API compatible with ioredis. This allows you to use familiar Redis patterns while connecting to your Redis.do deployment.

<Callout type="info">
The Redis.do client uses HTTP under the hood, not TCP. This means no connection pooling is required, and the client is fully stateless and safe to use in serverless environments.
</Callout>
...
interface RedisOptions {
  // Required: URL of your Redis.do deployment
  url: string
...
Redis.do is written in TypeScript and provides full type definitions.
...
The Redis.do client is stateless and uses HTTP for each request. There's no need to manage connections or implement reconnection logic.
...
While Redis.do is compatible with ioredis API patterns, some advanced features like transactions (MULTI/EXEC) and Lua scripting (EVAL) are not yet supported. Use pipelining for batch operations instead.
</Callout>
