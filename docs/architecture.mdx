...
description: Deep dive into Redois architecture including Durable Objects, sharding strategy, SQLite schema, and TTL management.
---

# System Architecture

Redois is built on Cloudflare's edge infrastructure, leveraging Durable Objects for consistent storage and Workers for request routing. This document explains the system architecture and design decisions.
...
Redois uses three types of Durable Objects, each with a specific responsibility.
...
Redois uses a fixed 256-shard architecture with consistent hashing for key distribution.
...
For commands that operate on multiple keys (MGET, DEL with multiple keys), Redois:
...
Redois implements key expiration using Cloudflare's Alarms API for efficiency.
...
3. RedoisEntrypoint.fetch() parses the path
...
2. Worker receives request
3. RedoisEntrypoint.fetch() routes to /rpc
...
2. Worker receives request
3. RedoisEntrypoint.fetch() routes to /mcp
